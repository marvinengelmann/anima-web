      ...
      <CardContent className="space-y-4">
        ...
        {data.ruminationDetected && (
          <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-destructive">{t("rumination")}</span>
              <Badge variant="destructive" className="text-xs">
                {t("ruminationTicks", { count: data.ruminationTicks })}
              </Badge>
            </div>
            {data.ruminationTopic && (
              <p className="mt-1 text-xs text-muted-foreground">{data.ruminationTopic}</p>
            )}
          </div>
        )}
        ...
        {data.complexDecisionCount > 0 && (
          <div className="flex items-center justify-between text-sm">
            ...
            <Badge variant="outline" className="text-xs">{data.complexDecisionCount}</Badge>
          </div>
        )}
      </CardContent>
    </Card>
